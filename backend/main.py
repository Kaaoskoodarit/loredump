...
@app.route('/api/worlds/<world_id>/lore_pages/<lore_page_id>', methods=['GET', 'PUT', 'DELETE', 'PATCH'])
...
def add_fake_data():
    ...
    try:
        ...
        user.login()
        return jsonify({'success': f'Fake data successfully added, logged in as {user.username}'}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 400
