...
router.get("/lorepage/:id",function(req,res) {
    // Find items belonging to a page NOTE! Use "_id" in backend, virtual fot front!
...
router.post("/lorepage",function(req,res) {
    ...
    // Check if request has a "body.type"...if no "body" at all, crashes, hence prev. test
    if(!req.body.title) {          
        return res.status(400).json({"Message":"Bad Request"})
    }
